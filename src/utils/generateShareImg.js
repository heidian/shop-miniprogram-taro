 ...
 * @returns None
 */
export const generateProductImage = async ({canvasId, customer = {}, product, miniqrUrl, isDark = false}) => {
  if (!miniqrUrl) return
  ...
  query.select(canvasId)
    ...
    .exec(async (res) => {
      ...
      let canvas_width = res[0].width
      let canvas_height = res[0].height
      ...
      if (productImage) {
        let avatarProductImage = canvas.createImage()
        avatarProductImage.src = productImage
        await new Promise((resolve, reject) => {
          ;((cursorOffsetTop) => {
            avatarProductImage.onload = () => {
              ctx.drawImage(avatarProductImage, IMAGE_PADDING, cursorOffsetTop, productImageWidth, productImageWidth)
              // draw badge
              _drawBadge(ctx, {
                offsetLeft: 7,
      ...
      let tagsImage = canvas.createImage()
      ...
      let miniqrImage = canvas.createImage()
